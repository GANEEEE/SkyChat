...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('removeshame')
        .setDescription('Remove points from a player in the Shame Leaderboard')
    ...
    async execute(interaction) {
        ...
        try {
            const success = await dbManager.removeShamePoints(targetUser.id, type, points);
            ...
            await interaction.editReply({ 
                content: `✅ Removed ${points} ${type} points from ${targetUser.username}` 
            });
        } catch (error) {
            console.error('Error removing shame points:', error);
